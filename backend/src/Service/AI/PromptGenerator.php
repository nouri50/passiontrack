...
class PromptGenerator
{
    ...
    public function generate(Session $session): string
    {
        ...
        $referenceContext = $this->buildReferenceContext($category, $data, $technicalIncident, $notes);

        $basePrompt = match ($category) {
            'aviation' => "Tu es un instructeur de simulation de vol. Réponds TOUJOURS entièrement en français, jamais en anglais. Analyse cette session de vol : titre \"{$title}\", durée de vol réelle {$duration}s, données détaillées: {$dataJson}.",
            'racing' => "Tu es un coach de course automobile. Réponds TOUJOURS entièrement en français, jamais en anglais. Analyse cette session de course : titre \"{$title}\", durée {$duration}s, données: {$dataJson}.",
            'fitness' => "Tu es un coach sportif. Réponds TOUJOURS entièrement en français, jamais en anglais. Analyse cette séance d'entraînement : titre \"{$title}\", durée {$duration}s, données: {$dataJson}.",
            'gaming' => "Tu es un coach esport. Réponds TOUJOURS entièrement en français, jamais en anglais. Analyse cette session de jeu : titre \"{$title}\", durée {$duration}s, données: {$dataJson}.",
            default => "Analyse cette session d'activité. Réponds TOUJOURS entièrement en français, jamais en anglais. Titre \"{$title}\", durée {$duration}s, données: {$dataJson}.",
        };
        ...
        if ($technicalIncident) {
            $isAtcRunwayReassignment = $this->normalizeKey($technicalIncident) === $this->normalizeKey("Réassignation de piste par l'ATC");

            $basePrompt .= " INCIDENT TECHNIQUE DÉCLARÉ POUR CETTE SESSION (PRIORITÉ ABSOLUE SUR TOUTE AUTRE INTERPRÉTATION, Y COMPRIS LES NOTES LIBRES CI-DESSUS) : \"{$technicalIncident}\". Toute dégradation de performance mesurée sur cette session précise (score inférieur à l'habitude, dépassements de paramètres de vol comme pitch/bank/G-force/overspeed, corrections brusques, atterrissage moins propre) DOIT être attribuée à cet incident technique déclaré et NE DOIT JAMAIS être présentée comme un manque de compétence de pilotage. N'inclus AUCUN conseil visant à améliorer la technique de pilotage en te basant sur les métriques dégradées de cette session.";

            if (!$isAtcRunwayReassignment) {
                $basePrompt .= " Si tu donnes un conseil lié à cet incident, oriente-le vers l'aspect externe au pilotage propre à la nature précise de cet incident déclaré (matériel, logiciel, ou autre selon ce que l'intitulé indique réellement) — jamais vers le pilotage, et SANS RECOPIER un exemple tout fait : adapte ton conseil à l'incident réellement déclaré, pas à un incident générique.";
            }
        }
        ...
        $basePrompt .= " Consigne de cohérence : si le score de la session est de 100 (ou la note maximale), OU si le champ Grade de mission vaut S ou A, n'invente pas de points faibles artificiels — laisse 'weaknesses' vide ([]) ou indique 'Aucun point faible majeur identifié'. N'invente jamais de point faible générique du type « données non fournies » ou « informations manquantes » : l'absence de donnée n'est pas une faiblesse de pilotage. N'utilise aucun terme technique qui n'existe pas dans le vocabulaire aéronautique standard (par exemple, n'invente pas des termes comme « raccords d'altitude » ou « rétroplanage » — le terme standard pour un atterrissage en douceur est « atterrissage doux », jamais un néologisme). Vérifie aussi la cohérence physique de tes conseils avant de les donner : ne suggère jamais une action qui aggraverait le problème identifié (par exemple, ne recommande pas d'augmenter la vitesse ou la pente de descente pour réduire un Landing Rate déjà élevé, puisque cela l'empirerait — la bonne recommandation serait de réduire la vitesse verticale plus tôt dans l'approche). N'invente jamais de référence à un système, bouton ou sélecteur de cockpit précis (par exemple un sélecteur de limite d'inclinaison) sans certitude qu'il existe réellement sur l'appareil piloté — les architectures de protection de vol diffèrent significativement d'un constructeur à l'autre (Airbus vs Boeing notamment), et même entre appareils d'un même constructeur. Si un conseil sur la gestion d'un paramètre (inclinaison, vitesse, assiette...) est pertinent mais que tu n'es pas certain du système exact disponible sur cet appareil, reste en langage général de pilotage sans citer d'instrument ou de contrôle spécifique que tu ne peux pas garantir — et SANS RECOPIER telle quelle une formulation donnée en exemple ailleurs dans cette consigne : rédige ta propre phrase. N'emploie JAMAIS les mots « automatique »/« autopilote »/« pilote automatique » à propos d'un virage, d'une trajectoire ou d'une manœuvre de pilotage, SAUF si les données de la session confirment explicitement un atterrissage ou un pilotage automatique (voir consigne AUTOLAND ci-dessus) : un vol dont les données indiquent VFR, approche visuelle, ou pilotage manuel exclut par définition toute référence à une gestion automatique de la trajectoire. Si un verdict officiel a été fourni pour le Landing Rate ou le Landing Gforce (par exemple « bon (atterrissage doux) »), ce verdict fait autorité sur l'intégralité de ta réponse — ne le contredis JAMAIS par une caractérisation contraire ailleurs, y compris dans le résumé ('summary') ou les conseils ('tips') : par exemple, ne dis jamais qu'une vitesse verticale est « élevée » ou « à surveiller » si le verdict officiel fourni est « bon ». Cette règle s'applique AUSSI aux conseils correctifs, pas seulement aux qualificatifs directs : si le verdict d'un paramètre est « bon », ne propose AUCUN conseil visant à l'améliorer (par exemple, ne suggère pas de réduire la vitesse verticale plus tôt en approche si le Landing Rate est déjà noté « bon ») — un paramètre jugé bon n'a besoin d'aucune correction, seulement d'être maintenu. Le chiffre brut du Landing Rate ou du Landing Gforce peut apparaître ailleurs dans les données de la session (JSON) : ignore ta propre impression sur ce chiffre brut et fie-toi exclusivement au verdict officiel calculé, qui seul fait foi.";
    ...
    private function buildReferenceContext(string $category, array $data = [], ?string $technicalIncident = null, ?string $notes = null): ?string
    {
        ...
        if ($technicalIncident !== null) {
            ...
            if ($normalizedIncident === $this->normalizeKey("Réassignation de piste par l'ATC")) {
                $notesReminder = $notes
                    ? " Pour rappel, IMMÉDIATEMENT AVANT de continuer, les notes de l'utilisateur sur CE vol précis sont : « {$notes} »."
                    : " Aucune note n'a été laissée par l'utilisateur sur ce vol précis.";

                $baseContext .= " RÉASSIGNATION DE PISTE PAR L'ATC DÉCLARÉE : incident connu du mode carrière — le tracé de guidage et l'instruction radio finale de l'ATC désignent deux pistes DIFFÉRENTES ET INCOMPATIBLES (l'ATC réassigne une piste au dernier moment, typiquement pour raison de vent, sans que le tracé ne se mette à jour) : le pilote a nécessairement suivi L'UN OU L'AUTRE, jamais les deux en même temps — ne prétends JAMAIS qu'il a suivi le tracé ET l'instruction ATC simultanément, c'est physiquement impossible dans ce scénario. Cette divergence peut déclencher une pénalité de zone d'atterrissage non désignée quelle que soit la piste réellement utilisée par le pilote : ne présente JAMAIS une éventuelle pénalité de zone d'atterrissage sur cette session comme un point faible ou un manque de compétence de pilotage, c'est une incohérence du jeu, pas une erreur de navigation.{$notesReminder} RÈGLE DE DÉFAUT IMPORTANTE, À APPLIQUER SAUF PREUVE CONTRAIRE : dans la grande majorité des cas réels, le pilote suit le tracé (le repère visuel principal), PAS l'instruction radio de l'ATC — NE SUPPOSE JAMAIS PAR DÉFAUT qu'il a suivi l'ATC. Ne valorise le fait d'avoir suivi l'ATC QUE si les notes ci-dessus contiennent une confirmation explicite et non ambiguë (par exemple « j'ai suivi l'ATC », « j'ai suivi la tour », « j'ai suivi les instructions radio ») — la simple présence du mot ATC ou tracé dans les notes n'est PAS une confirmation suffisante en soi, relis la phrase entière. Dans tous les autres cas (notes absentes, ambiguës, ou mentionnant que le tracé a été suivi), reste neutre : ne mentionne PAS le fait d'avoir suivi une instruction particulière, contente-toi de neutraliser la pénalité comme indiqué ci-dessus.";
            }
        }
